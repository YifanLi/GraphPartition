...
class PersonalizedPR {
  ...
  def hmMerge(dstID:VertexId, dstHM:HashMap[VertexId, Double], srcAtt: (Int,HashMap[VertexId,Double])) : HashMap[VertexId, Double] = {
  ...
  def sendMessage(edge: EdgeTriplet[(Int,HashMap[VertexId, Double]), Int]) =
    Iterator((edge.srcId, hmMerge(edge.dstId,edge.dstAttr._2,edge.srcAttr)))
