 ...
 * 5) the re-sort would be conducted on RDD, not in a TreeSet!(to avoid OOM)
 * 
 * Usage:
 ...
 * e.g.
 * nohup /usr/local/spark1/spark-1.4.1-bin-hadoop1/bin/spark-submit --driver-memory 40G --class "BlockedPartition_fixedWithoutTree" ~/graphx-montecarlo-fixedsizewithouttree_blockedpartition_2.10-0.1.jar 400 50G 300 8 0 LiveJournal 16 &
 */

object BlockedPartition_fixedWithoutTree {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("BlockPartition-fixedSizeWithoutTree_Yifan")
  ...
  val localPathToStorePartitions = "SizeFixedWithoutTree_"+ args(5) +"_"+numSeedFile+"_"+numIterations

  //the maximum size of intermediate arraybuffer in memory
  val maxSizeOfBF = 200000
...
//a function to divide the values in vertex's hashmap by its out-degree
  ...
  var gsItr = gsSorted.toLocalIterator
  ...
  //to store the edges will be removed from original RDD before encountering a "redundant" edge
  var tempAB = new ArrayBuffer[(((VertexId, Double),HashMap[VertexId, Double]),(VertexId, VertexId))]()
  ...
  while(gsItr.hasNext){
    var edgeWithHM = gsItr.next
    var seedID = edgeWithHM._1._1._1
    ...
    if(parBufs(seedBuffMapping.apply(seedID))==null){
    		  ...
    		  tempAB.append(edgeWithHM)
    		  ecounter = ecounter+1
    		  //take care...
    		  //edgeWithHM = gsItr.next
    		  hasPro = true
    		}
    
    //to check if current edge has been proceed
    if(hasPro && gsItr.hasNext){
      edgeWithHM = gsItr.next
      seedID = edgeWithHM._1._1._1
      }
    */
   // if(seedID == -1L){
   //   while(gsItr.hasNext){
   ...
   // }else{
    	//to indicate hasNext
    	var indt = true
    	while((parBufs(seedBuffMapping.apply(seedID))==null || seedID == -1L || parBufs(seedBuffMapping.apply(seedID)).size < maxSizeOfPar) && indt){
    		if(parBufs(seedBuffMapping.apply(seedID))==null){
    		  var tep = ArrayBuffer(edgeWithHM._2)
    		  parBufs(seedBuffMapping.apply(seedID)) = tep
    		  tempAB.append(edgeWithHM)
    		...
    		if(tempAB.size > maxSizeOfBF){
    		  var tempABrdd = sc.parallelize(tempAB, (numPartitions/10 + numPartitions%10))
    		  gsSorted = gsSorted.subtract(tempABrdd).sortBy(x => x._1._1._2, false)
    		  gsItr = gsSorted.toLocalIterator
    		  tempAB.clear
    	...
    	var newEdge = new ArrayBuffer[(((VertexId, Double),HashMap[VertexId, Double]),(VertexId, VertexId))]()
    	if(edgeWithHM._1._2.isEmpty){
    		  newEdge.append((((-1L, 0.0), HashMap[VertexId, Double]()),edge) )
    		}else{
    		  var newMax = edgeWithHM._1._2.maxBy(_._2)
    		  newEdge.append(((newMax, (edgeWithHM._1._2 -= newMax._1)),edge) )
    		}
    	//var newMax = edgeWithHM._1._2.maxBy(_._2)
    	//var newEdge = ArrayBuffer(((newMax, (edgeWithHM._1._2 -= newMax._1)),edge) )
    	var newRDD = sc.parallelize(newEdge)
    	
    	
    	
    	var tempABrdd = sc.parallelize(tempAB, (numPartitions/10 + numPartitions%10))
    	var gsSortedTem = gsSorted.subtract(tempABrdd)
    	gsSortedTem = gsSortedTem.union(newRDD)
    	
        var gsSortedT = gsSortedTem.sortBy(x => x._1._1._2, false)
        gsItr = gsSortedT.toLocalIterator
