...
/*
 * to analyze how many short random walks(SW) has crossed the boundary of partition that they belong
 * NOTICE:
 * in this experiments, the path of SW will not be recorded. Only the source walks ids have been transmitted.
 */

object PartitioningExperiments_CrossPartition {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("BlockPartitionExperimentsCrossPartition_Yifan")
  ...
  // to initialize the set of segments(source walks ids) from 1st randomly choose before pregel iteration computation
  def setInit(srcID:VertexId, tag: Int, num: Int):Set[(VertexId, (Int,Int))] ={
    val setOfsegs = Set[(VertexId, (Int,Int))]()
    if(num<1){
      return null
    }else{
      if(tag==1){
  ...
  def iniWalksDistribution(neighbors:Array[VertexId]):HashMap[VertexId,Set[(VertexId, (Int,Int))]] ={
    var disHM = HashMap.empty[VertexId,Set[(VertexId, (Int,Int))]]
    if(neighbors.isEmpty) return disHM
    for(e <- neighbors){
      disHM += (e -> Set.empty[(VertexId, (Int,Int))])
    }
    return disHM
  }
  ...
  */
  ...
  val firstRondomedVertexRDD = graph.collectNeighborIds(EdgeDirection.Out).map{case(id:VertexId, a:Array[VertexId]) => (id, (setInit(id,1,numOfSamplings), (a, iniWalksDistribution(a))))}
   ...
   *    (set of (srcVId,(PathId,SegmentId)), hashmap of to-be distributed messages)
   */
  val iniMessage = Set.empty[(VertexId, (Int,Int))]
  iniMessage += ((-10L,(0,0)))
  
  def vertexProgram(id: VertexId, attr: (Set[(VertexId, (Int,Int))],(Array[VertexId],HashMap[VertexId,Set[(VertexId, (Int,Int))]])), msgSum: Set[(VertexId, (Int,Int))]):(Set[(VertexId, (Int,Int))],(Array[VertexId],HashMap[VertexId,Set[(VertexId, (Int,Int))]]))={
    
      if(attr._2._1.length == 0){
        if(msgSum.size==1 && msgSum.head._1 == -10L){
          return attr
        }else{
        	return (attr._1++msgSum, (attr._2._1,HashMap.empty[VertexId,Set[(VertexId, (Int,Int))]]))
        }
      }else{
        if(msgSum.size==1 && msgSum.head._1 == -10L){
        	attr._1.foreach(x => {
        		val node = randomNeighbor(attr._2._1)
        		attr._2._2 += (node -> (attr._2._2.apply(node)))
        	}
        	)
        	return(Set.empty,attr._2)
        }else{
          attr._2._2.foreach(e => {
            attr._2._2 += (e._1 -> Set.empty[(VertexId, (Int,Int))])
          }
            )
          msgSum.foreach(y => {
            ...
            attr._2._2 += (nd -> (attr._2._2.apply(nd)))
          }
          )
  ...
  def sendMessage(edge: EdgeTriplet[(Set[(VertexId, (Int,Int))],(Array[VertexId],HashMap[VertexId,Set[(VertexId, (Int,Int))]])), Int]): Iterator[(VertexId, Set[(VertexId, (Int,Int))])]={
    //val dis = edge.srcAttr._2._2.apply(edge.dstId)
    if(!edge.srcAttr._2._2.contains(edge.dstId)){
      Iterator.empty
    }else{
      if(edge.srcAttr._2._2.apply(edge.dstId).isEmpty){
        Iterator.empty
      }else{
    	  Iterator((edge.dstId,edge.srcAttr._2._2.apply(edge.dstId)))
    }
  }
  
  def messageCombiner(a:Set[(VertexId, (Int,Int))], b:Set[(VertexId, (Int,Int))]):Set[(VertexId, (Int,Int))] = {
