...
/*
 * (Notice: this class is to compute PPR with global convergence)
 * 
 ...
 */
class PPR_Opti_counters_global {
  ...
  def sendMessage(edge: EdgeTriplet[(Int,HashMap[VertexId, Array[Double]]), Int]) = {
    val hm = hmMerge(edge.dstId,edge.dstAttr._2,edge.srcAttr._1)
    println("$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
    println("size: ")
    println(hm.size)
    println("~~~~")
    println(hm)
    println("$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
    if(hm.size.equals(1) && hm.contains(-100L)){
      println("11111")
      Iterator.empty
    }else{
      println("22222")
      Iterator((edge.srcId, hm))
    }
  }
  ...
  val ppr = initialGraph.pregel(initialMsg = iniMessage, activeDirection = EdgeDirection.In)(vertexProgram, sendMessage, messageCombiner)
