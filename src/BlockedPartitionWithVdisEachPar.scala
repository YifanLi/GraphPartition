 ...
 * 
 * And, to store:
 * 1) each vertex's distances to every seed
 * 2) the mapping from edge to seed
 * 
 * Usage:
 ...
 * e.g.
 * nohup /usr/local/spark1/spark-1.5.2-bin-hadoop1/bin/spark-submit --driver-memory 40G --class "BlockedPartitionWithVdisEachPar" ~/graphx-bpwvep_2.10-0.1.jar 400 45G 300 6 5 LiveJ 10 &
 */

object BlockedPartitionWithVdisEachPar {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("BlockedPartitionWithVdisEachPar_Yifan")
...
//a function to divide the values in vertex's hashmap by its out-degree
  ...
  //an rdd to store the distances to every seed for each vertex
  val vDisToSeedsRdd = ppr.vertices.map(e => (e._1, e._2._2._1))
  //vDisToSeedsRdd.saveAsObjectFile("vDisToSeedsRdd"+"_"+args(5)+"_numPars"+numPartitions.toString)
  ...
  vDisToSeedsRdd.saveAsTextFile("vDisToSeedsRdd"+"_"+args(5)+"_numSeeds"+seedFiles(numSeedFile).toString())
  
  //an rdd to store the mapping from an edge to a seed
  //note: here we need only one edge to indicate this partition's seedID
  val edMapSeed = gsGroups.map(x => (x._2.head, x._1))
  //edMapSeed.saveAsObjectFile("edMapSeed"+"_"+args(5)+"_numPars"+numPartitions.toString)
  
  ///e.g. an element in below RDD(derived from text file):
  //((12,13),-1)
  //note: Long, Double, blankspace(using str.trim())
  edMapSeed.saveAsTextFile("edMapSeed"+"_"+args(5)+"_numSeeds"+seedFiles(numSeedFile).toString())
