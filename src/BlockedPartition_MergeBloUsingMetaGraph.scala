 ...
 * 4) TO merge blocks into partitions using the new meta-graph idea, rather than 4/3 theorem
 * 5) launch random walks for every node(corresponds block) in meta-graph to find the merging 
 * 6) Notice:
 * - the link attribute in meta-graph is the intersection of vertices of linked blocks(vertex in meta-graph)
 * 
 * Usage:
 * => ./bin/spark-submit --class BlockedPartition ~/BlockPartition_v1.jar [partitions] [spark.executor.memory] [spark.cores.max] [iterations] [seeds file] [Pokec/LiveJ] [topk] [numOfPartitionsMerged] [interFac]
 * e.g.
 * nohup /usr/local/spark/spark-1.6.2-bin-hadoop1/bin/spark-submit --driver-memory 20G --class "BlockedPartition_MergeBloUsingMetaGraph" ~/graphx-montecarlo-blocksmerge-metagraph_2.10-0.1.jar 400 30G 200 4 2000 LiveJ 5 200 0.15 &
 * nohup /usr/local/spark/spark/bin/spark-submit --driver-memory 20G --class "BlockedPartition_MergeBloUsingMetaGraph" ~/graphx-montecarlo-blocksmerge-metagraph_2.10-0.1.jar 400 30G 200 4 2000 LiveJ 5 200 0.15 &
 * 
 */

object BlockedPartition_MergeBloUsingMetaGraph {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("BlockPartition-BlocksMergeUsingMetaG_Yifan")
  ...
  //teleportation constant(decay factor in computing closeness)
  val ct = 0.15
  ...
  //the max percentage of intersection between two blocks when initialize the P
  val interFac = args(8).toDouble
...
//a function to divide the values in vertex's hashmap by its out-degree
  ...
  //a hashmap to store this iterator
  var blocSizes = new HashMap[Long, Int]()
  while(countEdgesItr.hasNext){
    var blocIdSize = countEdgesItr.next
    blocSizes += (blocIdSize._1 -> blocIdSize._2)
  }
  
  
  //to get the vertices set of each block
  var verSetB = groupedGS.map(x => (x._1, x._2.flatMap(x => Array(x._2._1, x._2._2)).toSet ))
  //to remove the "-1" block from above temporarily
  var verSetBlock = verSetB.filter(_._1 != -1L)
  //a hashmap to store all the (block_id, vertices_set) above
  var bloSets = new HashMap[Long, Set[VertexId]]()
  var itr = verSetBlock.toLocalIterator
  while(itr.hasNext){
    var (id, set) = itr.next
    bloSets += (id -> set)
  }
  ...
  //an array[arraybuffer] to store every partition(blocks) scheduling result
  //(num of edges in all, block ids, the set of vertices in all )
  //var ParsArr = new Array[(Int, ArrayBuffer[Long], Set[VertexId])](numOfPars)
  var ParsArr = Array.fill[(Int, ArrayBuffer[Long], Set[VertexId])](numOfPars)((0, ArrayBuffer[Long](), Set[VertexId]()))

  //a set to keep the blocks assigned
  //var assBlocks = Set[VertexId]()
  
  /*
   * NOTICE:
   * here, for the first imported blocks into P, candidate partitions,
   * the intersection between each two candidates should be LESS than a pre-fixed factor, say interFac,
   * to avoid two initial partitions(blocks) are "too close".
   */
  //to find the first imported blocks into P
  var bs = countEdges.toLocalIterator
  var i = 0
  while(bs.hasNext){
    var idSiz = bs.next
    var blockID = idSiz._1
    var blockSize = idSiz._2
    
    var tep = ArrayBuffer(blockID)
    var verSet = verSetBlock.filter(_._1 == blockID).first._2
    
    if(ParsArr.isEmpty){
      ParsArr(i) = (blockSize, tep, verSet)
      //assBlocks += blockID
      bloSets.remove(blockID)
      
    }else{
      var ind = true
      for(x <- ParsArr){
        var setA = x._3
        //to check if the intersection is too large 
        //which means the new candidate block is too close to one or more blocks in P
        var inter = setA & verSet
        if((inter.size.toDouble/setA.size > interFac) || (inter.size.toDouble/verSet.size > interFac)){
          ind = false
        } 
      }
      if(ind){
  ...
  //to check if the number of expected initial blocks is enough
  if(ParsArr.last._3.isEmpty){
    println("The initial blocks imported into P is not enough! Do reduce the factor interFac!")
    //sc.stop
    //return
  }
  
  while(!bloSets.isEmpty){
    //1st: to find the smallest candidate partition p in P
    var iSmall = 0
    var sizeSmall = ParsArr(0)._1
    var bls = ParsArr(0)._2
    var setSmall = ParsArr(0)._3
    for(i <- 1 to (ParsArr.length-1)){
      if(ParsArr(i)._1 < sizeSmall){
    	  iSmall = i
    	  sizeSmall = ParsArr(i)._1
    	  bls = ParsArr(i)._2
    	  setSmall = ParsArr(i)._3
      }
    }
    
    //2nd: to find all the un-assigned blocks having max intersection(POC) with above p
    //var blockSets = verSetBlock.toLocalIterator
    var blockInters = new Array[(Long, Int)](bloSets.size)
    var i = 0
    for((bid, bset) <- bloSets){
      //var (bid, bset) = blockSets.next
      var inter = bset & setSmall
      blockInters(i) = (bid, inter.size)
      i = i+1
    }
    //to get the max-intersected block(s)
    var maxInts = blockInters.groupBy(_._2).maxBy(_._1)._2
    //to get the biggest one in above block(s)
    //(block_id, sizeOfVertices, sizeOfEdges)
    var tt = maxInts.map(x => (x._1, x._2, blocSizes(x._1)))
    var maxInt = tt.maxBy(_._3)
    
    var newSet = setSmall | bloSets(maxInt._1)
    bls.append(maxInt._1)
    ParsArr(iSmall) = (sizeSmall+maxInt._3, bls, newSet)
    
    bloSets.remove(maxInt._1)
    
    
    
  }
  
  //to convert ParsArr into blocksArr by removing vertices_set
  var blocksArr = ParsArr.map(x => (x._1, x._2))
  ...
  for(i <- 0 to (sortedBlocksArr.length-1)){
    ...
    val writer = new PrintWriter(new File("GraphPartition_MetaGraph_"+i.toString+"_"+args(5)+"_"+sizeOfPar.toString+".txt" ))
