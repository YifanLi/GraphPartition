 ...
 * 5) ONLY part of edges, e.g. 10%, will be scanned for re-scheduling
 * 
 * Usage:
 * => ./bin/spark-submit --class BlockedPartition ~/BlockPartition_v1.jar [partitions] [spark.executor.memory] [spark.cores.max] [iterations] [seeds file] [Pokec] [topk] [pathToStorePartitions]
 * e.g.
 * nohup /usr/local/spark1/spark-1.4.1-bin-hadoop1/bin/spark-submit --driver-memory 30G --class "BlockedPartition_fixedBS_Part" ~/graphx-montecarlo-fixedsize_part_blockedpartition_2.10-0.1.jar 300 50G 300 8 _keepNeigh_16 LiveJournal 16 partitions &
 * nohup /usr/local/spark1/spark-1.4.1-bin-hadoop1/bin/spark-submit --class "BlockedPartition_fixedBS_Part" ~/graphx-montecarlo-fixedsize_part_blockedpartition_2.10-0.1.jar 300 20G 300 8 _keepNeigh_16 LiveJournal 16 partitions &
 */

object BlockedPartition_fixedBS_Part {
...
//a function to divide the values in vertex's hashmap by its out-degree
  ...
  //to filter edges by seedId
  for(x <- seedBuffMapping){
    println("seed id: "+x._1.toString)
    if(x._1 == -1L){
      var negOneRdd = toGroupGS.filter(_._1 == -1L)
      println("the count of edges: "+negOneRdd.count.toString)
      
      if(negOneRdd.count > 0){
      var negOneRddMap = negOneRdd.map(x => x._2._2)
      var negOneRddItr = negOneRddMap.toLocalIterator
      var tep = ArrayBuffer(negOneRddItr.next)
      parBufs(seedBuffMapping.apply(-1)) = tep
      while(negOneRddItr.hasNext){
        parBufs(seedBuffMapping.apply(-1)).append(negOneRddItr.next)
      }
      }
    }else{
      var someEdgesRdd = toGroupGS.filter(_._1 == x._1)
      println("the count of edges: "+someEdgesRdd.count.toString)
      
      if(someEdgesRdd.count > 0){
        var someEdgesRddMap = someEdgesRdd.map(x => (x._2._2, x._2._1))
        var sortedSomeEdgesRdd = someEdgesRddMap.sortBy(x => x._2, false, (numPartitions/(hubRDD.count.toInt) + numPartitions%(hubRDD.count.toInt)))
        var toTakeSorted = sortedSomeEdgesRdd.take(maxSizeOfSchel)
        var te = new ArrayBuffer[(VertexId, VertexId)]()
        parBufs(seedBuffMapping.apply(x._1)) = te
        var TTSorted = toTakeSorted.map(x => x._1)
        parBufs(seedBuffMapping.apply(x._1)) ++= TTSorted
        
        if(sortedSomeEdgesRdd.count > maxSizeOfSchel){
          var toTakeSortedRdd = sc.parallelize(toTakeSorted)
          var restEdges = sortedSomeEdgesRdd.subtract(toTakeSortedRdd).map(x => (x._1, 0))
          reSchEdgesRDD = reSchEdgesRDD.union(restEdges)
...
/*  
  ...
  while(groupedItr.hasNext){
    indd = indd+1
    println("indd --------> "+indd.toString)
    
    var seedAndEdges = groupedItr.next
    
    if(seedAndEdges._1 == -1L){
      if(!seedAndEdges._2.isEmpty){
      var saeIterator = seedAndEdges._2.toIterator
      var tep = ArrayBuffer(saeIterator.next._2)
      parBufs(seedBuffMapping.apply(-1)) = tep
      while(saeIterator.hasNext){
        var saeNext = saeIterator.next._2
        parBufs(seedBuffMapping.apply(-1)).append(saeNext)
      }
      }
    }else{
      if(!seedAndEdges._2.isEmpty){
        
        println("size of seedAndEdges._2 : "+seedAndEdges._2.size.toString)
        
        var saeArray = seedAndEdges._2.toArray
        var sortedsae = saeArray.sortBy(_._1).map(x => x._2)
        var sortedsaeTR = sortedsae.takeRight(maxSizeOfSchel)
        
        var te = new ArrayBuffer[(VertexId, VertexId)]()
        parBufs(seedBuffMapping.apply(seedAndEdges._1)) = te
        parBufs(seedBuffMapping.apply(seedAndEdges._1)) ++= sortedsaeTR
        
        if(sortedsae.size > maxSizeOfSchel){
          reSchEdges ++= sortedsae.take(sortedsae.size - maxSizeOfSchel)
        }
        
        var partRDD = sc.parallelize(reSchEdges).map(x => (x, 0))
        println("partRDD.size:  "+partRDD.count.toString)
        reSchEdgesRDD = reSchEdgesRDD.union(partRDD)
        reSchEdges.clear
...
*/  
  ...
  //var pairReSchE = reSchEdges.map(x => (x, 0))
  
  println("222222222222222")
  //var reSchEdgesRDD = sc.parallelize(pairReSchE)
  
  //gs = reversedGS.join(reSchErdd).map(x => ((x._2._1),x._1))
  gs = reversedGS.join(reSchEdgesRDD).map(x => ((x._2._1),x._1))
  
  //=========print some info========
  println("=========print some info========")
  println("maxSizeOfPar: "+maxSizeOfPar.toString)
  println("reSchErdd: "+reSchEdgesRDD.count.toString)
  for(bf <- parBufs){
    println("bf ---> "+bf.size.toString)
  }
  println("=========print some info========")
