 ...
 * 
 * Usage:
 * => ./bin/spark-submit --class FPPRMC ~/FPPRMC_v1.jar [partitions] [spark.executor.memory] [max.cores] [partitionStrategy] [defaultOrBlocked] [PartitonsToLoad:201;298;300] [numOfPagerankIteration]
 * e.g.
 * nohup /usr/local/spark1/spark-1.5.2-bin-hadoop1/bin/spark-submit --driver-memory 30G --class "PartitioningExperiments_StaticPageRank" ~/graphx-staticpagerank_2.10-0.1.jar 204 30G 150 2 PowerGraphPartitioning LiveJournal1_PowerGraph200 30 &
 */

object PartitioningExperiments_StaticPageRank {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("StaticPageRank_Yifan")
