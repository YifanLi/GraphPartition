 ...
 * 5) all (duplicated-)edges will be sorted!
 * 
 * Usage:
 * => ./bin/spark-submit --class BlockedPartition ~/BlockPartition_v1.jar [partitions] [spark.executor.memory] [spark.cores.max] [iterations] [num of seed file] [Pokec] [topk] [facOfParl]
 * e.g.
 * nohup /usr/local/spark1/spark-1.4.1-bin-hadoop1/bin/spark-submit --driver-memory 30G --class "BlockedPartition_fixedFullSort" ~/graphx-montecarlo-fixedfullsort_blockedpartition_2.10-0.1.jar 400 50G 300 6 3 LiveJournal 20 6 &
 */

object BlockedPartition_fixedFullSort {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("BlockPartition-fixedFullSort_Yifan")
    conf.set("spark.ui.port","4000")
    conf.set("spark.master","spark://small1-tap1:7077")
    //be careful!!
    conf.set("spark.local.dir","/data2")
    ...
    //conf.set("spark.kryoserializer.buffer.max.mb", "5120")
  ...
  //
  val facOfParl = args(7).toInt
  
  //path to store the partitioning result
  val localPathToStorePartitions = "SizeFixedFullySorted_"+ args(5) +"_"+numSeedFile+"_"+numIterations
...
//a function to divide the values in vertex's hashmap by its out-degree
  ...
  val maxSizeOfPar = edgeRdd.count/hubRDD.count.toInt + 1
  ...
  //an array to indicate each arraybuf's size in above
  //var bfSizes = new Array[Long](hubRDD.count.toInt+1)
  ...
  //to unfold all the edges' distance values in an RDD
  val allEdgesRdd = edgeRdd.flatMap(e => {if(e._2.isEmpty) Buffer(((-1L, 0.0), e._1)) else e._2.map(sv => ((sv._1, sv._2), e._1)).toBuffer+=(((-1L, 0.0), e._1))})
  //to sort it above
  val sortedAllEdgesRdd = allEdgesRdd.sortBy(x => x._1._2, false).repartition(numPartitions*(topk*facOfParl)).persist(MAD)
  val sortedAEItr = sortedAllEdgesRdd.toLocalIterator
  
  println("----> the initial number of edges: "+edgeRdd.count.toString)
  println("----> the count of all unfolded edges: "+ allEdgesRdd.count.toString)
  println("the number of partitions in edges_rdd: "+sortedAllEdgesRdd.partitions.size.toString)
  //println("the size of each partition in edges_rdd:")
  //sortedAllEdgesRdd.foreachPartition(x => println(x.size))
  //var dd = sortedAllEdgesRdd.mapPartitions(x => Buffer(x.size).toIterator)
  //for(x <- dd.toArray){
  //  println(x)
  //}
  ...
  //to store all the edges which have been allocated appropriately.
  var allocatedEdges = new HashSet[(VertexId, VertexId)]()
  
  //to count the processed edges in sortedAEItr
  var ecounter = 0
  
 
  /*
   * to scan the edges RDD(sortedAEItr)
   */
  while(sortedAEItr.hasNext){
    ecounter = ecounter+1
    
    var edge = sortedAEItr.next
    var seedID = edge._1._1
    var eIDs = edge._2
    if(!allocatedEdges.contains(eIDs)){
      if(parBufs(seedBuffMapping.apply(seedID))==null){
    		  var tep = ArrayBuffer(eIDs)
    		  parBufs(seedBuffMapping.apply(seedID)) = tep
    		  allocatedEdges += eIDs
    		  
    		  //println("111111111111111")
      }else{
        if((parBufs(seedBuffMapping.apply(seedID)).size < maxSizeOfPar) || (seedID == -1L)){
    		parBufs(seedBuffMapping.apply(seedID)).append(eIDs)
    		allocatedEdges += eIDs
    ...
    if(ecounter%100000 == 0){
      println("------")
      println(ecounter)
      println("the num of allocated edges: "+allocatedEdges.size.toString)
      for(x <- parBufs){
        if(x != null){
        println("size of edge_array: "+x.size.toString)
        }else{
          println("a null!")
        }
  ...
  println("-------------- edges in -1 block re-allocation is done!!! ----------") 
  ...
  for(x <- hubRDD.toArray){
    ...
    */
    ...
    val writer = new PrintWriter(new File(localPathToStorePartitions+"_"+x._1.toString+".txt" ))
