 ...
 * 
 * Usage:
 * => ./bin/spark-submit --class FPPRMC ~/FPPRMC_v1.jar [partitions] [spark.executor.memory] [max.cores] [partitionStrategy] [defaultOrBlocked] [PartitonsToLoad:201;298;300] [PageRank_tol]
 * e.g.
 * nohup /usr/local/spark1/spark-1.4.1-bin-hadoop1/bin/spark-submit --driver-memory 30G --class "PartitioningExperiments_DynamicPageRank" ~/graphx-dynamicpagerank_2.10-0.1.jar 204 30G 150 2 PowerGraphPartitioning LiveJournal1_PowerGraph200 0.005 &
 */

object PartitioningExperiments_DynamicPageRank {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("FPPRMC_DynamicPR_Yifan")
    ...
    val numParName = args(5).toString()
    //input files on HDFS
    //val edgesFileOnHDFS = "hdfs://small1-tap1.common.lip6.fr:54310/user/yifan/soc-PokecRelationships.txt"
    var edgesFileOnHDFS = ""
    ...
    println("=====> The edges file on HDFS:")
    println(edgesFileOnHDFS)
  ...
  //to indicate the variables in Dynamic PageRank
  val tol = args(6).toDouble
  val resetProb = 0.15
  ...
  //val ppr = graph.staticPageRank(args(6).toInt, 0.15).vertices.take(5)(1)
  val ppr = graph.pageRank(tol, resetProb)
