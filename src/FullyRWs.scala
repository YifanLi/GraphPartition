 ...
 * 
 * Usage:
 * => ./bin/spark-submit --class FPPRMC ~/FPPRMC_v1.jar [partitions] [spark.executor.memory] [max.cores] [numIterations] [partitionStrategy] [defaultOrBlocked] [numOfSamplings] [PartitonsToLoad:201;298;300;Pokec_PowerGraph200]
 * e.g.
 * nohup /usr/local/spark/spark-1.6.2-bin-hadoop1/bin/spark-submit --driver-memory 30G --class "FullyRWs" ~/graphx-fullyrws_2.10-0.1.jar 205 30G 150 10 2 blockedPartitioning 1 LiveJ_MerBlo200 &
 */
...
object FullyRWs {
  ...
  val firstRondomedVertexRDD = graph.vertices.map{case(id:VertexId, a:Int) => (id, setInit(id,1,numOfSamplings))}.persist(MAD)
