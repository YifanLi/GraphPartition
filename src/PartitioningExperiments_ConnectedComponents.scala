 ...
 * 
 * Usage:
 * => ./bin/spark-submit --class FPPRMC ~/FPPRMC_v1.jar [partitions] [spark.executor.memory] [max.cores] [partitionStrategy] [defaultOrBlocked] [PartitonsToLoad:201;298;300; Pokec100refined] 
 * e.g.
 * nohup /usr/local/spark1/spark-1.4.1-bin-hadoop1/bin/spark-submit --driver-memory 30G --class "PartitioningExperiments_ConnectedComponents" ~/graphx-connectedcomp_2.10-0.1.jar 67 30G 150 2 PowerGraphPartitioning LiveJournal1_PowerGraph64 &
 */

object PartitioningExperiments_ConnectedComponents {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("FPPRMC_CC_Yifan")
  ...
  val ppr = graph.connectedComponents.vertices.first
  println(ppr)
