...
object BlockedPartition_fixedWOTree {
...
//a function to divide the values in vertex's hashmap by its out-degree
   ...
   */
  while((gs.count > 0) && (ecounter<30)){
    ecounter = ecounter+1
    
    var emaxList = gs.top(1)(Ordering.by(_._1._1._2))
    gs = gs.subtract(sc.parallelize(emaxList))
    
    var edgeWithHM = emaxList(0)
    var seedID = edgeWithHM._1._1._1
    
    //println("#################################")
    //println("edgeWithHM: "+edgeWithHM)
    //println("(srcID : tarID): "+edgeWithHM._2)
    //println("seedID: "+seedID.toString)
    //println("gs.count: "+gs.count.toString)
    //println("#################################")
    ...
    //println("----------> ecounter:   "+ ecounter.toString)
  }
